package com.rentas.ppob.trans.bpjs.ketenagakerjaan;
...
public class BpjsKetenagakerjaanActivity extends MyActivity {
    ...
    protected void initLayout() {
        ...
        header_view.create("Bpjs Ketengakerjaan");
    ...
    private void showNominal(){
        ...
        int[] nominals = {1, 3, 6, 12};
        for (int value : nominals) {
            Bundle bundle = new Bundle();
            bundle.putInt("qty",value);
            bundle.putString("name", value+"");
            bundle.putString("year", "Bulan");
            nominal.add(bundle);
        }
        ...
        post.setOnReceiveListener((obj, code, message) -> {
            if (code == ErrorCode.OK){
                try {
                    ...
                    long cba = data.getLong("customer_bill_amount");
                    long cutBalance = data.getLong("cut_balance");
                    admin = cutBalance - cba;

                    intent.putExtra("customer_bill_amount",cba);
                    intent.putExtra("price",cutBalance);
